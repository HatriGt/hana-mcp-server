...
const log = (msg) => console.error(`[HANA MCP Server] ${new Date().toISOString()}: ${msg}`);

log('Starting HANA MCP Server with fallback...');
...
async function getHanaClient() {
  ...
  if (!process.env.HANA_HOST || !process.env.HANA_USER || !process.env.HANA_PASSWORD) {
    log('Missing HANA configuration');
    return null;
  }
...
// Tool implementations with fallback to mock data
const toolImplementations = {
  hana_show_config: async (args) => {
    ...
    const status = (configInfo.host !== 'NOT SET' && configInfo.user !== 'NOT SET' && process.env.HANA_PASSWORD ? 'PROPERLY CONFIGURED' : 'MISSING REQUIRED VALUES');
  ...
  hana_describe_table: async (args) => {
    ...
    try {
      ...
      const columnsText = results.map(col => {
        ...
        const dataType = col.DATA_TYPE_NAME + (col.LENGTH ? `(${col.LENGTH})` : '') + (col.SCALE ? `,${col.SCALE}` : '');
        
        return `${col.COLUMN_NAME.padEnd(20)} | ${dataType.padEnd(12)} | ${(col.LENGTH || '-').toString().padEnd(6)} | ${nullable.padEnd(8)} | ${defaultValue.padEnd(8)} | ${description}`;
      }).join('\n');
  ...
  hana_list_indexes: async (args) => {
    ...
    try {
      ...
      const results = await client.query(query, [schema_name, table_name]);
      ...
      const indexesText = Object.entries(indexMap).map(([indexName, index]) => {
        const type = index.isUnique ? 'Unique' : index.type;
        const columns = index.columns.join(', ');
        return `- ${indexName} (${type}) - Columns: ${columns}`;
      }).join('\n');
        ...
        content: [
          {
            ...
            text: `📋 Indexes for table '${schema_name}.${table_name}':\n\n` +
                  `${indexesText}\n\n` +
                  `Total indexes: ${Object.keys(indexMap).length}`
          }
        ]
      };
    } catch (error) {
  ...
  hana_describe_index: async (args) => {
        ...
        content: [
          {
            ...
            text: `❌ Error: schema_name, table_name, and index_name parameters are required\n\n` +
                  `Usage: Provide schema_name, table_name, and index_name to describe the index.`
          }
        ]
      };
    }
    ...
    try {
      ...
      const results = await client.query(query, [schema_name, table_name, index_name]);
        ...
        content: [
          {
            ...
            text: `❌ Error: query parameter is required\n\n` +
                  `Usage: Provide a SQL query to execute.`
          }
        ]
      };
    }
      ...
      case 'initialize':
        ...
        return {
          ...
          result: {
            ...
            serverInfo: {
              name: 'HANA MCP Server (with fallback)',
              version: '1.0.0'
            }
          }
        };
